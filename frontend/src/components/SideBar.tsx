    ...
    return (
        <div className="text-[#e5e5e5] flex sticky top-0 h-screen">
            <aside className={`hidden md:flex flex-col border-r border-white/5 transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] ${isSidebarCollapsed ? 'w-18' : 'w-60'}`}>
                ...
                <div className="p-3 border-t border-white/3 flex flex-col gap-1.5 shrink-0">
                    {/* Theme Toggle */}
                    <button
                        onClick={toggleDarkMode}
                        className={`flex items-center h-9.5 px-3 rounded-xl text-white/25 hover:text-white/70 hover:bg-white/1.5 transition-all duration-300 ${isSidebarCollapsed ? 'justify-center' : 'justify-start'}`}
                    ...
                    <div className={`flex items-center mt-2 px-3 py-2.5 rounded-xl border border-transparent hover:border-white/4 bg-white/1 hover:bg-white/2 transition-all duration-300 ${isSidebarCollapsed ? 'justify-center' : 'justify-between'}`}>
                        <Link to="profile" className="flex items-center gap-3">
                            ...
                            <span className={`text-xs font-medium text-white/60 whitespace-nowrap transition-opacity duration-200 ${isSidebarCollapsed ? 'hidden' : 'block'}`}>
                                Dipanshu Sahu
                            </span>
                        </Link>
                        {!isSidebarCollapsed && (
                            <button onClick={() => setIsSidebarCollapsed(true)} className="text-white/20 hover:text-white/80 transition-colors p-1" aria-label="Collapse Sidebar">
                                <LuPanelRightClose size={14} />
                            </button>
                        )}
                    </div>
                    {isSidebarCollapsed && (
                        <button onClick={() => setIsSidebarCollapsed(false)} className="mt-1 w-full flex justify-center text-white/20 hover:text-white/80 transition-colors py-2" aria-label="Expand Sidebar">
            ...
            <nav className="md:hidden fixed bottom-0 left-0 w-full h-20 border-t border-white/5 bg-[#050505]/80 backdrop-blur-xl flex items-center justify-around px-4 sm:px-6 z-50">
                {navItems.map((item) => {
                    const Icon = item.icon;
                    return (
                        <NavLink
                            ...
                            key={item.route}
                            className={({ isActive }) => (
                                `flex flex-col items-center justify-center gap-1.5 transition-colors p-2 w-16 sm:w-20 ${isActive ? 'text-white drop-shadow-[0_0_8px_rgba(255,255,255,0.3)]' : 'text-white/30'
                                }`
                            )}>
                            <Icon size={22} className='opacity-80' />
                            <span className="text-[9px] sm:text-[10px] font-medium tracking-wider uppercase opacity-80">
                                {item.label}
                            </span>
                        </NavLink>
                    );
